...
- Returns list with results.\033[0m
    ...
    try:
        for i in range(len(height)):
            if isinstance(height[i], int) == False and isinstance(height[i], float) == False:
                raise AssertionError(height[i])
        for i in range(len(weight)):
            if isinstance(weight[i], int) == False and isinstance(weight[i], float) == False:
                raise AssertionError(weight[i])
        if len(height) != len(weight):
            raise AssertionError("\033[1;31mAssertionError: Both lists need to be the same size !\033[0m")
        ...
        print("\033[1;31mAssertionError: '",error, "' is neither a int or a float !\033[0m", sep="")
...
- Returns bool list with results.\033[0m
    ...
    try:
        if len(bmi) == 0 or bmi is None:
            raise AssertionError("\033[1;31mAssertionError: BMI list must not be empty!\033[0m")
        if not all(isinstance(x, (int, float)) for x in bmi):
            raise AssertionError("\033[1;31mAssertionError: BMI list must not be empty!\033[0m")
        if isinstance(limit, int) == False:
            raise AssertionError("\033[1;31mAssertionError: limit argument must be a positive int!\033[0m")
        if not all(x >= 0 for x in bmi) or limit < 0:
            raise AssertionError("\033[1;31mAssertionError: bmi and limit must be positive!\033[0m")
...
def main():
    height = [65]  # Liste contenant des entiers
    height.append(58)  # Liste contenant des entiers
    # height = ["test"]
    print("height:", height)
    
    weight = [180]  # Liste contenant des entiers
    weight.append(110)
    # weight.append(75)
    print("weight:", weight)
